...
export function buildProceduralGlasses(spec: FrameSpec, shape: FrameShape = 'square'): { root: THREE.Group; anchor: AssetAnchor } {
  const g = buildFrameGeometry({ shape, spec });
  ...
  const v = (p: number[]): [number, number, number] => [p[0], p[1], p[2]];
  return {
    root,
    anchor: { bridge: v(g.anchors.bridge), temple_left: v(g.anchors.temple_left), temple_right: v(g.anchors.temple_right), nose_pad_offset: v(g.anchors.nose_pad_offset) },
  };
}
